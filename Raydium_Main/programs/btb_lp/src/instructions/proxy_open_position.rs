...
pub struct ProxyOpenPosition<'info> {
 ...
 //dev
    #[account(
        ...
        bump,
        payer = payer,
        space = 8 + 1  // 8 bytes for discriminator + 1 byte for initialized flag
    )]
    pub btb_pda_account: Account<'info, BtbPdaState>,
...
pub fn proxy_open_position<'a, 'b, 'c: 'info, 'info>(
    ...
    if !ctx.accounts.btb_pda_account.is_initialized {
        // Set initialized flag
        ctx.accounts.btb_pda_account.is_initialized = true;
        msg!("PDA Account initialized for the first time");
    }
