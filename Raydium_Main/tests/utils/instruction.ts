...
export async function openPosition(
  ...
  const [btbPdaAccount] = PublicKey.findProgramAddressSync(
    [
      Buffer.from("btb_pda_account"),
      owner.publicKey.toBuffer()
    ],
    program.programId
  );

  console.log("pda account **** " , btbPdaAccount )
  ...
  console.log("Ata account **** " , positionANftAccount);
  ...
  const tx = await program.methods
    ...
    .accounts({
      btbPdaAccount: btbPdaAccount,
      clmmProgram: ClmmProgram,
      payer: owner.publicKey,
      positionNftOwner: btbPdaAccount,
